...
def create_face_encodings(folder= "facial_images"):
    for filename in os.listdir(folder):
        if filename.endswith(".jpg") or filename.endswith(".png"):
            ...
            face_buffer.append([config.buffer_duration, "None"])
...
with mp_face_detection.FaceDetection(
 ...
 while cap.isOpened():
    ...
    for i in range(0, len(face_buffer)):
        if (face_buffer[i][0] < config.buffer_duration):
            face_buffer[i][0] = face_buffer[i][0] + 1
...
#             print(keypoint_classifier_labels[hand_sign_id])
    ...
    #         print('distance is')
#             print(dist)
            if(dist < config.threshold and face_recognition_accuracy > config.face_match_threshold and matches[best_match_index]):
                if not config.buffer:
                    recognition_response(face_label, keypoint_classifier_labels[hand_sign_id], datetime.datetime.now().strftime("%m-%d-%Y %H:%M:%S"))
                    # print(face_label, keypoint_classifier_labels[hand_sign_id])
                else:
                    if(face_buffer[best_match_index][0] == config.buffer_duration):
                        recognition_response(face_label, keypoint_classifier_labels[hand_sign_id], datetime.datetime.now().strftime("%m-%d-%Y %H:%M:%S"))
                        face_buffer[best_match_index][0] = 0
                    else:
                        if(not face_buffer[best_match_index][1] == keypoint_classifier_labels[hand_sign_id]):
                            recognition_response(face_label, keypoint_classifier_labels[hand_sign_id], datetime.datetime.now().strftime("%m-%d-%Y %H:%M:%S"))
                            face_buffer[best_match_index][0] = 0

                    face_buffer[best_match_index][1] = keypoint_classifier_labels[hand_sign_id]
